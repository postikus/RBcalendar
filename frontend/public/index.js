...
(function ($public, $window, $, undefined) {
    ...
    _private.CalendarObj.prototype.change_hash = function(__hash_object){
        ...
        window.location.hash = __hash_object.month
            ...
            + '/' + __hash_object.view_mode;
    };

    _private.CalendarObj.prototype.get_hash_object_from_url = function(){
        var hash_object = {month: undefined, year: undefined, filters: undefined, opened_event: undefined, search_string: undefined, view_mode: undefined};
        ...
        hash_object.view_mode = hash.length > 5 ? hash[5] : '';
        return hash_object;
    };

    _private.CalendarObj.prototype.get_search_hash_object_from_url = function(){
        var search_string = '';
        var hash = window.location.hash.replace('#', '').split('/');
        search_string = hash.length > 4 ? hash[4] : '';
        return search_string;
    };
    _private.CalendarObj.prototype.get_view_mode_hash_object_from_url = function(){
        var view_mode = '';
        var hash = window.location.hash.replace('#', '').split('/');
        view_mode = hash.length > 5 ? hash[5] : '';
        return view_mode;
    };

    _private.CalendarObj.prototype.set_search_to_empty = function(){
        var this_hash = this.get_hash_object_from_url();
        this_hash.search_string = '';
        this.change_hash(this_hash);
    };
	...
	_private.CalendarObj.prototype.opacity_change = function(){
        ...
        var hash_obj = this.get_hash_object_from_url();
        hash_obj.search_string = $('#rb_calendar_search').val();
        this.change_hash(hash_obj);
        var reg = new RegExp(hash_obj.search_string,"ig");
        $('.calendar-event').not('.list-box__item').map(function(){
    ...
    //_private.CalendarObj.prototype.set_events = function (events_array){
    ...
    _private.CalendarObj.prototype.spiner = function( target, visibility ) {
        //cl('spiner start')
        var target = target || document.body || document.querySelector( 'body' );
        var visibility = ( visibility ) ? true : false;
        if ( !target.hasAttribute( 'data-hasspiner' ) ) {
            target.setAttribute( 'data-hasspiner', '' );
            var spiner = document.createElement( 'div' );
            spiner.setAttribute( 'data-spiner', '' );
            spiner.setAttribute( 'data-spiner-show', 'false' );
            spiner.innerHTML = '<div data-spiner-overlay></div><div data-spiner-circle></div>';
            //target.append( spiner );
            $('.spiner-container').html(spiner);
        } else  {
            var spiner = target.querySelector( '[data-spiner]' );
        }
        if ( visibility === true ) spiner.setAttribute( 'data-spiner-show', 'true' )
            else spiner.setAttribute( 'data-spiner-show', 'false' );

    };

    _private.CalendarObj.prototype.render_list = function(events_array, calendar_obj){
        //this.set_events(events_array);
        calendar_obj.events = events_array;
        cl('list render start', events_array);
        var this_event;
        var calendar_html = '';
        calendar_html += '  <div class="row justify-content-center month-select-container">';
        calendar_html += '    <h3 class="col-12"></h3>';
        calendar_html += '  </div>';
        calendar_html += '  <div class="list-box">';
        //_private.CalendarObj.prototype.set_popup_html(events_array);

        for (var _event_count=0; _event_count < events_array.length; _event_count++){
            this_event = events_array[_event_count];
            calendar_html += '    <div class="list-box__item calendar-event"'
            +'data-event-type="'+this_event.type+'"'
            +'data-registred="'+this_event.registred+'"'
            +'data-in_wlist="'+ this_event.in_wlist +'" '
            +'data-is_open="'+this_event.is_open+'"'
            +'data-beexpert="'+this_event.beexpert+'"'
            +_private.CalendarObj.prototype.get_data_element_html(this_event, 'hub')
            +_private.CalendarObj.prototype.get_data_element_html(this_event, 'type_detailed')
            +_private.CalendarObj.prototype.get_data_element_html(this_event, 'for_type')
            +_private.CalendarObj.prototype.get_data_element_html(this_event, 'event_rb_type')+'>';
            calendar_html += '      <div class="list-box__item-header">';
            calendar_html += '        <span class="calendar-event-name">'+this_event.name+'</span><div class="calendar-event-pers" title="free space"><div class="calendar-event-icons text-right">'+this_event.max_pers+' <i class="fa fa-users"></i></div></div><label for="modalTrigger" data-modalbtn="" class="calendar-event_btn" data-idx="'+_event_count+'" data-id="'+this_event.id+'" data-opt-event-type="'+this_event.type+'">Подробнее</label>';
            calendar_html += '      </div>';
            calendar_html += '      <div class="list-box__item-body">';
            calendar_html += '        <span class="list-box__item-param"><strong>Дата начала: </strong><span>'+_private.CalendarObj.prototype.format_date(this_event.start_date)+'</span></span>';
            calendar_html += '        <span class="list-box__item-param"><strong>Дата завершения: </strong><span>'+_private.CalendarObj.prototype.format_date(this_event.finish_date)+'</span></span>';
            calendar_html += '      </div>';
            calendar_html += '    </div>';
        }
        ...
        // calendar_html += '  <div class="list-box" style="text-align:center">';
        // calendar_html += '      <div data-p-btn data-p-btn-left title="prev">&lt;</div>'
        // calendar_html += '      <div data-p-btn data-p-btn-num>1</div>';
        // calendar_html += '      <div data-p-btn data-p-btn-right class="enabled" title="next">&gt;</div>';
        // calendar_html += '  </div>';


        
        $('.list-container').html(calendar_html);

        return calendar_html;
    }


    _private.CalendarObj.prototype.get_list_html = function(calendar_obj, _hash_obj){
        _private.CalendarObj.prototype.spiner(0, 1);
        var html_str = '';
        // var filter_string = [];
        // for (var _fc = 0; _fc < _hash_obj.filters.length; _fc++ ){
            // filter_string.push();
        // }
        cl(_hash_obj)
        $.ajax({
            url: "./custom_web_template.html?object_id=6684913066409134747&"+_hash_obj.filters.join('&'), //dev
            // data: _hash_obj,
            data: {search_string: _hash_obj.search_string},
            dataType: 'json',
            contentType: 'charset=win-1251',
            cache: false,
            async: true
        }).done(function(data) {
            // cl('done')
            html_str += _private.CalendarObj.prototype.render_list(data, calendar_obj);
            _private.CalendarObj.prototype.spiner(0, 0);
        });
    ...
    _private.CalendarObj.prototype.get_calendar_view_button_container = function () {
        ...
        __calendar_view_button_html += '<label for="triggerVievLayout" data-calendar><i class="fas fa-th calendar-view-button"></i></label>';
        __calendar_view_button_html += '<label for="triggerVievLayout" data-list><i class="fas fa-list list-view-button"></i></label>';
    ...
    _private.CalendarObj.prototype.get_filters_html = function(){
        ...
        __filters_html += _private.CalendarObj.prototype.get_filters_row_container('checkboxs', {
            ...
            label: 'Тип:',
            checkbox_array:[
                {id: 'type_training', label: 'Тренинг'},
    ...
    _private.CalendarObj.prototype.get_legend_html = function(){
        return '<div class="row"><div class="col-12"><div class="legend-list">'
    ...
    _private.CalendarObj.prototype.render_calendar = function (callback) {
        ...
        $(__self.mount_id+' .calendar-container').html(__self.get_calendar_html(__self));
		callback();
    };
    ...
    _private.CalendarObj.prototype.format_date = function( _date ){
    ...
    _private.CalendarObj.prototype.set_popup_html = function(__events, _target, _open){
        ...
        function createContent( idx ) {
            ...
            var mMainInnerHtml = '';
			// cl('hub', obj.hub.split(';'));
			var hub_array = [];
			for (var hcounter=0; hcounter < obj.hub.split(';').length; hcounter++){
				// cl(obj.hub.split(';')[hcounter].split("]["), obj.hub.split(';')[hcounter].split("][").length);
				if (obj.hub.split(';')[hcounter].split("][").length >= 2){
					hub_array.push( obj.hub.split(';')[hcounter].split("][")[1] );
				}
				else{
					hub_array.push( obj.hub.split(';')[hcounter] );
				}
			}
            ...
            mMainInnerHtml = '<div class="m-cell-1">'
                ...
                +'<p>' + (obj.local_time_string ? obj.local_time_string + '<br>' : '') + format_date(obj.start_date) + '</p>'
                //+'<p>' + format_date(obj.start_date) + '</p>'
                +'<div class="m-title">Дата завершения:</div>'
                +'<p>' + (obj.local_time_string ? obj.local_time_string + '<br>' : '') + format_date(obj.finish_date) + '</p>'
	...
	if (obj.ood == 0){				
            if (obj.is_open == 0){
		footer_html += '<b>'+obj.type_no_access+'</b>';
            }
	    else if (obj.registred == 1){
			if (obj.price){
				if (daysBetween(Date(), obj.start_date) > 4){
					footer_html +='<button class="btn m-btn m-btn_black" id="unreg-button" data-out="'+(obj.price ? 1 : 0)+'" data-id="'+obj.id+'" data-idx="'+idx+'">Отменить регистрацию</button>';
				}
				else{
					footer_html += '<span style="font-weight:700">Для отмены регистрации свяжитесь с сотрудниками ДКО</span>';
				}
			}
			else{
				footer_html +='<button class="btn m-btn m-btn_black" id="unreg-button" data-out="'+(obj.price ? 1 : 0)+'" data-id="'+obj.id+'" data-idx="'+idx+'">Отменить регистрацию</button>';
            ...
            else if(obj.in_wlist == 1 || obj.in_wlist == "1"){
		footer_html +='Ваша заявка обрабатывается';
	    }
            else if(obj.max_pers == 0 || obj.max_pers == "0"){
		footer_html +='<button class="btn m-btn m-btn_black" id="wlist-button" data-out="'+(obj.price ? 1 : 0)+'" data-id="'+obj.id+'" data-idx="'+idx+'">Лист ожидания</button>';
	    }
	    else if (obj.registred == 0){
		footer_html +='<button class="btn m-btn m-btn_black" id="reg-button" data-out="'+(obj.price ? 1 : 0)+'" data-id="'+obj.id+'" data-idx="'+idx+'">Зарегистрироваться</button>';
            }
    ...
    _private.CalendarObj.prototype.get_data_element_html = function (___event, _name) {
        var _data_element_html = '';
        var _event_data_attribute_array = [];
        if (___event[_name]){
            ___event[_name].split(';').map(function(__ev){
    ...
    _private.CalendarObj.prototype.render_events = function (__events) {
        ...
        var get_event_html = function ( idx ) {
            ...
            var __event_html = '<div data-event-type="' + __events[idx].type + '" ' +
                ...
                'data-in_wlist="'+ __events[idx].in_wlist +'" ' +
    ...
    $public.init = function(args) {
        ...
        // cl('go')
        $(document).on('change', $('#rb_calendar_search'), function(){
            new_calendar.opacity_change();
            if (new_calendar.get_hash_object_from_url().view_mode === 'list' && ($('#rb_calendar_search').val().length > 3 || $('#rb_calendar_search').val().length === 0)){
                // cl('oo')
                new_calendar.get_list_html(new_calendar, new_calendar.get_hash_object_from_url());
            }
        });
        ...
        $(document).on('click','[data-calendar]', function (){
            $public.get_events(new_calendar).then( function( resp ){
                ...
                new_calendar.render_events(events);
                $('.calendar-event').removeClass('d-none');
                new_calendar.opacity_change();
            } ).catch( function( e ){ console.error( e ); } );
            var this_hash = new_calendar.get_hash_object_from_url();
            this_hash.view_mode = 'calendar';
            new_calendar.change_hash(this_hash);
        });

        $(document).on('click','[data-list]', function (){
            var this_hash = new_calendar.get_hash_object_from_url();
            this_hash.view_mode = 'list';
            new_calendar.get_list_html(new_calendar, this_hash);
            new_calendar.change_hash(this_hash);
        });
        ...
        $(document).on('click','#reg-button', function () {
            ...
            var ev_out = $(this).attr('data-out');
            //cl(__event_idx);
            $.ajax({
                url: "./custom_web_template.html?object_id=6675296127857605162", //dev
                data: {event_id: __event_id, user_id: $('#curUserID').val(), method: 'reg', is_out: ev_out},
                // dataType: 'json',
                // contentType: 'charset=win-1251',
                cache: false,
                async: true
	    ...
	    var ev_out = $(this).attr('data-out');
            ...
            $.ajax({
                url: "./custom_web_template.html?object_id=6675296127857605162", //dev
                data: {event_id: __event_id, user_id: $('#curUserID').val(), method: 'unreg', is_out: ev_out},
                // dataType: 'json',
                // contentType: 'charset=win-1251',
                cache: false,
                async: true
        ...
        $(document).on('click','#wlist-button', function () {
            var __event_id = $(this).attr('data-id');
            var __event_idx = $(this).attr('data-idx');
	    var ev_out = $(this).attr('data-out');
            //cl(__event_id);
            $.ajax({
                url: "./custom_web_template.html?object_id=6685420753029179544", //dev
                data: {event_id: __event_id, user_id: $('#curUserID').val(), method: 'reg', is_out: ev_out},
                cache: false,
                async: true

            }).done(function(data) {
                new_calendar.events[__event_idx].in_wlist = 1;
                $('.calendar-event[data-id="'+__event_id+'"]').attr('data-in_wlist', 1);
                new_calendar.opacity_change();
                $( '#reg-container' ).html( data );
            });
        });
        ...
        var make_url = function(is_inited){
            this_hash_obj = {opened_event: '', month: new_calendar.date_month, year: new_calendar.date_year, filters: [], search_string: new_calendar.get_search_hash_object_from_url(), view_mode:new_calendar.get_view_mode_hash_object_from_url()};
            if (is_inited){
            ...
            $('#rb_calendar_search').val(decodeURI(this_hash_obj.search_string));

            var new_hash_obj = {month: this_hash_obj.month, year: this_hash_obj.year, filters: checked_cb_ids, opened_event: this_hash_obj.opened_event, search_string: this_hash_obj.search_string, view_mode: this_hash_obj.view_mode};
            if (this_hash_obj.view_mode === 'list'){
                // cl('wa');
                $('#triggerVievLayout').prop('checked', false);
                new_calendar.get_list_html(new_calendar, new_hash_obj);
            }
        ...
        $(_options.mount_id).on('click','#filters-clear',function () {
            ...
            // $('#rb_calendar_search').val('');
            new_calendar.set_search_to_empty();
            make_url();
...
if (hash_obj.view_mode === 'list'){
        $.ajax({
            url: "./custom_web_template.html?object_id=6684913066409134747&"+hash_obj.filters.join('&'), //dev
            // data: _hash_obj,
            data: {search_string: hash_obj.search_string},
            dataType: 'json',
            contentType: 'charset=win-1251',
            cache: false,
            async: false
...
cl(new_calendar.events)
        ...
        $(_options.mount_id).on('change','.form-check-input',function () {
            // cl('change');
            make_url();
            // if (new_calendar.get_hash_object_from_url().view_mode === 'list'){
            //     new_calendar.render_list(new_calendar.get_hash_object_from_url());
            // }
        });
