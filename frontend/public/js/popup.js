    ...
    */
    ...
    if ("onpropertychange" in modalTrigger) {
        ...
        modalTrigger.onpropertychange = function() {
            // проверим им€ изменЄнного свойства
...
/* типа обработчик клика и эмул€ци€ запроса */
;( function() {
    ...
    function createContent( idx ) {
        ...
        // console.log( idx, obj );
        // console.log( replacer("&laquo;–ец&lsquo;&rsquo;епт&amp;amp;©успе&bull;шнойЃпрезе&ldquo;нт&rdquo;ац&bdquo;ии&deg;от&nbsp;ай&mdash;&hellip;ай&lt;ен&gt;ан&trade;ка&raquo;") );
        ...
        mMain.innerHTML = '<div class="m-cell-1">'
            ...
            //+'<p>' + obj.type + '</p>'
            +'<div class="m-title">ƒата начала:</div>'
            +'<p>' + format_date(obj.start_date) + '</p>'
            +'<div class="m-title">ƒата завершени€:</div>'
            +'<p>' + format_date(obj.finish_date) + '</p>'
            +'<div class="m-title">—вободных мест:</div>'
            +'<p>' + obj.max_pers + '</p>'
            //+'<div class="m-title">Price:</div>'
            //+'<p>' + obj.price + '</p>'
            +'<div class="m-title">ѕреподаватель:</div>'
        ...
        if (obj.is_open == 0){
            ...
            footer_html +='<button class="btn m-btn m-btn_black" id="reg-button" data-id="'+obj.id+'" data-idx="'+idx+'">«арегистрироватьс€</button>';
        } 
        else if (obj.registred == 1){
            footer_html +='<button class="btn m-btn m-btn_black" id="unreg-button" data-id="'+obj.id+'" data-idx="'+idx+'">ќтменить регистрацию</button>';
        }
        footer_html +='</div>'
            +'<div class="m-cell-2">'
            /*+'<button class="btn m-btn m-btn_white">«арегистрироватьс€</button>    '*/
            +'</div>';
